...
from lib.train.dataset import SV248S_TRAIN,VISO_TRAIN,VISO_VAL
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full",
                        ...
                        "REFCOCOG", "TNL2K_train", "OTB99_train",'SV248S_TRAIN','VISO_TRAIN','VISO_VAL']
        # if name == "LASOT":
        ...
        if name == "VISO_TRAIN":
            if settings.use_lmdb:
                print("Building viso_train dataset from lmdb")
                datasets.append(VISO_TRAIN(settings.env.viso_train_dir, image_loader=image_loader))
            else:
                datasets.append(VISO_TRAIN(settings.env.viso_train_dir, image_loader=image_loader))
        if name == "VISO_VAL":
            if settings.use_lmdb:
                print("Building viso_val dataset from lmdb")
                datasets.append(VISO_VAL(settings.env.viso_val_dir, image_loader=image_loader))
            else:
                datasets.append(VISO_VAL(settings.env.viso_val_dir, image_loader=image_loader))
    return datasets
