...
class VISO_TRAIN(BaseVideoDataset):
    """ VISO_TRAIN dataset.
    """
    ...
    def _read_bb_anno(self, seq_path):
        try:
            bb_anno_file = os.path.join(seq_path, "new_groundtruth.txt")
            gt = pandas.read_csv(bb_anno_file, delimiter=',', header=None, dtype=np.float32, na_filter=False,
                                low_memory=False, skipinitialspace=True).values
        except ValueError:
            print(bb_anno_file,'=======')
    ...
    def _get_sequence_path(self, seq_id):
        ...
        return os.path.join(self.root, seq_name, 'img')
